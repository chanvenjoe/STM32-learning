...
void led_init(void)
{
	...
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA|RCC_AHBPeriph_GPIOB|RCC_AHBPeriph_GPIOC|RCC_AHBPeriph_GPIOD, ENABLE); //ALL port clock enabled
...
//    GPIO_SetBits(LED_FAULT_PORT, LED_FAULT_PIN);
    GPIO_ResetBits(LED_EN_PORT, LED_EN_PIN); //Light up the LED
