...
bit DanWei3_40,DanWei3_65,DanWei2_40;

bit jia_close_led_fl,huanxing_fl;

unsigned char sw_songtimer;
	
bit duzhuan_30min_fl;
unsigned char duzhuan_cishu_num;
bit duzhuan_cishu_fl;	
bit jia_zheng_close_fl;	
   ...
   if((BeginMoveBackTM>100)||(BeginMoveFrontTM>100))            //50ms/0.5MS=100
      {	
			...
			if(PWM_xianzhi_fl==0)
             ...
             {								
               ...
               PWM=101;               //40%
							...
							if(PWM<76&&DanWei==2)      //30%   
               PWM=76; 
							
							if((PWM<=102&&DanWei==3)||(PWM<=78&&DanWei==2))   //进入限流调整 25A 且PWM小于40%时 76=30%    102=40%
							{
	   ...
	   else	 
       {
		  ...
		  if(TIME_20MS_CT>=41U)//20MS ???     电源指示灯
		  {
            TIME_20MS_CT=0;	
            ...
            if(VoltageLowFL)
                {
                 ...
                 if(ELE30_BIG_ERR_FL||ELE20_BIG_ERR_FL||ELE28_BIG_ERR_FL)
                {
                 LED_CT=6;	 //3次
                }
                 else
  ...
  EADC=0;   
	...
	//==============================  
	    if(POWER_on_TM<POWER_ON_TM)
         ...
         if(((Cur_ResultNew>=73)&&(DanWei==1))||((Cur_ResultNew>=73)&&(DanWei3_40==1))||((Cur_ResultNew>=73)&&(DanWei2_40==1))) //一档    //25A=73    73=1.41V      
              {
              ...
              else
                {
					...
					if(((Cur_ResultNew>=115)&&(DanWei==2))||((Cur_ResultNew>=115)&&(DanWei3_65==1))) //二档    //35A=100   1.96V     40A=115=2.25V
              {
                ELE13_BIG_ct++;
                ELE13_Small_ct=0;
                if(ELE13_BIG_ct>5)
	...
	//------------------------------------------------------	
 /* if(Cur_ResultNew > ELE60A&&DanWei==3)           //三档堵转保护     50A=2.81V=143
  {
	  	LED1_IO=~LED1_IO;			                     //700us左右
	...
	//	{
		...
		//		{
		 ...
		 //}
			...
			{
			   if(ADtemp<47)           //10%---40%  对应的电压
				 {
					 	PWM_HallSet=80;       //就是40%
					 DanWei3_40=1;
					 DanWei3_65=0;
				 }					 
				if(ADtemp>=47&&ADtemp<86)     //40%---65%  对应的电压
				{
			    PWM_HallSet=143;        //就是65%
					DanWei3_65=1;
					 DanWei3_40=0;
				}
					if(ADtemp>=86)        //大于65%    对应的电压
					{
			      PWM_HallSet=ADtemp*PWM_98_DV/HALL_DV;      //有霍尔变速
						 DanWei3_40=0;
						 DanWei3_65=0;
					}
			}
		
		 if((HIGH_LOWS_PEED_FL==0)&&(DanWei==2))    //二档  65%
		 {
		    if(ADtemp<95)       //10%--50%  统一40%
				{
				    PWM_HallSet=80;       //就是40%
					  DanWei2_40=1;
				}
			...
			if((HIGH_LOWS_PEED_FL==0)&&(DanWei==1))    //一档   40%
				PWM_HallSet=80;                             
			//PWM_HallSet=ADtemp*PWM_40_DV/HALL_DV; 

			if((HIGH_LOWS_PEED_FL==0)&&(DanWei==0))    //倒档   40%
			PWM_HallSet=ADtemp*PWM_40_DV/HALL_DV; 
...
void zidongguangji()
{
	...
	if(jia_close_led_fl==0)
	{
		...
		if(jia_close_led_fl)      //当堵转保护计时灯灭后   按一下重新唤醒
		{
		   if(huanxing_fl==0)
				{
						if(SW_IO)
